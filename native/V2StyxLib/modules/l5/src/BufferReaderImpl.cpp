...
styxlib::StyxString BufferReaderImpl::readUTFString()
{
    if (position + sizeof(uint16_t) > limit)
    {
        throw std::out_of_range("Buffer overflow");
...
styxlib::Size BufferReaderImpl::read(uint8_t *data, styxlib::Size count)
{
    if (position + count > limit)
    {
        throw std::out_of_range("Buffer overflow");
    }
    for (styxlib::Size i = 0; i < count; ++i)
    {
