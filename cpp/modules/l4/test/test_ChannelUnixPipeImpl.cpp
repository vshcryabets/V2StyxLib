...
public:
    ...
    void connectClient() {
        auto fds = server->getFileDescriptors();
        REQUIRE(fds.readFd != -1);
        REQUIRE(fds.writeFd != -1);
...
// TEST_CASE_METHOD(TestSuiteUnixPipe, "Server accepts connections", "[ChannelUnixPipe]")
// {
//     waitStartServer();
